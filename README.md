...
座標で指定した地点は名前を付けて端末内に**最大5件保存**でき、山名欄の候補から1タップで呼び出せます
（沢登りの入渓点など、山名の無い場所を何日も続けて確認する用途向け）。
保存した地点は最後に見た予報も地点ごとに残るので、圏外でも開けます。
...
**暗い配色**: 端末(OS)を夜モードにしていると、画面も暗い配色になります。
各ページのフッタにある「自動 / ライト / ダーク」で、端末の設定とは別にこの画面だけを
固定することもできます(既定は「自動」＝端末の設定に従う。選んだ状態はその端末に保存されます)。
表示している内容と登山指数の判定は明るい配色のときと同じです。
