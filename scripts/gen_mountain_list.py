...
.searchcard{{background:var(--surface);border-radius:14px;padding:14px 16px;
  box-shadow:0 2px 12px var(--shadow);position:sticky;top:8px;z-index:10}}
.searchcard label{{display:flex;flex-direction:column;gap:5px;font-size:.82em;font-weight:600;color:var(--muted)}}
.searchcard input{{font-size:16px;padding:11px 12px;border:1.5px solid var(--field);border-radius:10px;
  background:var(--surface);width:100%;font-family:inherit}}
.searchcard input:focus{{outline:2px solid var(--night);outline-offset:1px}}
#hits{{margin:8px 0 0;font-size:.82em;color:var(--muted);font-weight:600}}
...
h2{{color:var(--head);font-size:1.08em;margin:24px 0 8px;border-left:5px solid var(--sky);padding-left:8px}}
h2 .cnt{{color:var(--muted);font-size:.85em;font-weight:600}}
.tbl{{overflow-x:auto;-webkit-overflow-scrolling:touch;margin:6px 0 4px}}
table{{border-collapse:collapse;width:100%}}
th{{background:var(--slate);color:var(--on-night);padding:6px 9px;font-weight:600;font-size:.9em}}
td{{padding:7px 9px;border-bottom:1px solid var(--line);text-align:center;background:var(--surface)}}
