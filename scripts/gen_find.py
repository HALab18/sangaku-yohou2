...
(function(){
  ...
  var DEGRADED_LEVEL_IDX=LEVELS.reduce(function(a,L,li){
    if(L[0]===900||L[0]===800)a.push(li);return a},[]);
  ...
  // 次にバージョンを上げる時は、この配列に現行の "find12" を追記すること。
  var OLD_FIND_PREFIXES=["find2","find3","find4","find5","find6","find7","find8","find9","find10","find11"];
  ...
  function render(rows,date,gotAt,initAll){
    ...
    // 稜線風が900hPa/800hPa欠測(GSM期間・5日目以降)でやや弱めに出ている山も、表の下に1回だけ明記する。
