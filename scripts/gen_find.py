...
.tbl{overflow-x:auto;-webkit-overflow-scrolling:touch;margin:10px 0 4px}
table{border-collapse:collapse;width:100%}
th{background:var(--slate);color:#fff;padding:6px 8px;font-weight:600;font-size:.86em;white-space:nowrap}
...
td.nm{text-align:left;white-space:normal}
...
(function(){
  ...
  function dispWx(s){
    ...
    if(f==null){
      var c=s.code;
      var ic=c==null?"·":c>=95?"⛈":(c>=71&&c<=77||c===85||c===86)?"❄️":
        (c>=51&&c<=67||c>=80&&c<=82)?"🌧":(c===45||c===48)?"🌫":c===3?"☁️":c===2?"🌤":"☀️";
      return {ic:ic,lb:wlabel(c)};
    }
    if(f>=0.80)return {ic:"☀️",lb:"よく晴れ"};
    if(f>=0.55)return {ic:"🌤",lb:"晴れ"};
    if(f>=0.30)return {ic:"⛅",lb:"時々晴れ"};
    return {ic:"☁️",lb:"曇りがち"};
  }
  ...
  function render(rows,date){
    ...
    rows.forEach(function(row,i){
      ...
      h+='<tr>'+
        ...
        '<td><span class="wx">'+wx.ic+'</span><br><small>'+esc(wx.lb)+'</small></td>'+
