...
<!-- ブラウザのUI(アドレスバー)の色も配色に合わせる -->
<meta name="theme-color" media="(prefers-color-scheme:light)" content="#1e2d4a">
<meta name="theme-color" media="(prefers-color-scheme:dark)" content="#101725">
<!-- 配色(明るい配色と、端末が夜モードのときの暗い配色)。色の値はここが唯一の置き場で、
     各ページのスタイル指定は var(--◯◯) しか書かない。?v= は theme.css の --pw-theme-ver と
     同じ値にする(古い版がキャッシュに残ると「画面は新しいのに配色だけ旧版」になる)。
     一致は scripts/check_contrast.py が機械的に見ている。 -->
<link rel="stylesheet" href="../theme.css?v=248">
<!-- 配色の切り替え(自動/ライト/ダーク)。既定は「自動」＝端末(OS)の設定に従う。
     ★ defer を付けず、theme.css の直後で **同期実行** すること。遅らせると
       明るい配色で一度描いてから暗転する(起動時の白い一瞬が戻る)。 -->
<script src="../theme.js?v=248"></script>
...
/* ★ 色の値をここに書かないこと。全部 theme.css のトークンを var() で参照する。
   ここに :root{--night:…} を書き戻すと theme.css より後に来るため上書きになり、
   **暗い配色が丸ごと効かなくなる**(明るい配色では正常に見えるので気づけない)。 */
*{box-sizing:border-box}
body{margin:0;background:var(--bg);color:var(--text);
  font-family:"Hiragino Kaku Gothic ProN","Yu Gothic UI","Meiryo",system-ui,sans-serif;
  font-size:14px;line-height:1.6}

header{background:var(--night);color:var(--on-night);text-align:center;padding:30px 20px 0;position:relative}
header .back{position:absolute;left:14px;top:12px;color:var(--on-night-2);font-size:.82em;text-decoration:none}
header .back:hover{color:var(--on-night)}
header h1{margin:0;font-size:1.45em;font-weight:800;letter-spacing:.04em}
header .sub{margin:8px 0 0;font-size:.85em;color:var(--on-night-2)}
.ridge{display:block;width:100%;height:44px;margin-top:18px}
/* 山稜の切り抜きは「ヘッダの下の地面」なので、地の色に追従させる。
   SVG の fill="#f4f6f9" は theme.css が読めなかったときの控えで、通常はこちらが勝つ
   (これが無いと、暗い配色でヘッダの下に明るい帯が1本残る)。 */
.ridge path{fill:var(--bg)}

main{max-width:860px;margin:0 auto;padding:18px 14px 8px}

.searchcard{background:var(--surface);border-radius:14px;padding:14px 16px;
  box-shadow:0 2px 12px var(--shadow)}
.searchcard .row{display:flex;flex-wrap:wrap;gap:12px}
/* min-width:0 は flex 子要素のデフォルト(auto)を無効にして親幅を尊重させる */
.searchcard label{display:flex;flex-direction:column;gap:5px;font-size:.82em;font-weight:600;color:var(--muted);flex:1 1 150px;min-width:0}
.searchcard select,.searchcard input{font-size:16px;padding:11px 12px;border:1.5px solid var(--field);border-radius:10px;
  background:var(--surface);width:100%;max-width:100%;min-width:0;font-family:inherit}
.searchcard select:focus,.searchcard input:focus{outline:2px solid var(--night);outline-offset:1px}
.searchcard .go{margin-top:12px;width:100%;background:var(--btn);color:var(--on-night);border:0;border-radius:10px;
  padding:13px;font-size:1.02em;font-weight:700;font-family:inherit;cursor:pointer}
.searchcard .go:hover{background:var(--btn)}
.searchcard .go:disabled{background:var(--btn-off);cursor:default}
.hint{margin:10px 0 0;font-size:.8em;color:var(--muted)}

#status{margin:14px 0 0;font-size:.9em;font-weight:600;min-height:1.2em}
#status.err{color:var(--bad-fg)}

.notice{background:var(--surface-3);border-left:5px solid var(--rule);color:var(--head-2);padding:10px 12px;
...
th{background:var(--slate);color:var(--on-night);padding:6px 8px;font-weight:600;font-size:.86em;white-space:nowrap;position:sticky;top:0;z-index:2}
td{padding:8px;border-bottom:1px solid var(--line);text-align:center;background:var(--surface);white-space:nowrap}
...
.abc-a{background:var(--ok-bg);color:var(--ok-fg)}
.abc-b{background:var(--warn-bg);color:var(--warn-fg)}
.abc-c{background:var(--bad-bg);color:var(--bad-fg)}
.abcr{display:block;font-size:.72em;color:var(--bad-fg);font-weight:700;margin-top:2px;white-space:nowrap}
...
.find-toggle{width:100%;padding:9px;font-size:.92em;font-weight:600;border-radius:10px;
  margin:2px 0 14px;background:var(--surface);color:var(--link);border:1.5px solid var(--line)}
.find-toggle:hover{background:var(--surface-4)}
...
td.nm .scb.rank-a{color:var(--ok-fg)}
td.nm .scb.rank-b{color:var(--warn-fg)}
td.nm .scb.rank-c{color:var(--bad-fg)}
...
tr:nth-child(even) td.rank,tr:nth-child(even) td.nm{background:var(--surface-2)}
...
tr.row-c td{background:var(--bad-bg)}
tr.row-c td.rank,tr.row-c td.nm{background:var(--bad-bg)}
...
td.nm small.brk{color:var(--muted-2);font-size:.78em;letter-spacing:.02em}
...
tr[data-href]:hover td,tr[data-href]:hover td.rank,tr[data-href]:hover td.nm{background:var(--hover)}
/* 結果ブロックの見出しと注記 (メイン表 / 足切り表を分ける) */
h3.results-h{margin:18px 0 4px;font-size:1em;color:var(--head);font-weight:700}
h3.results-h.caution{color:var(--warn-fg)}
h3.results-h .rcount{color:var(--muted);font-weight:500;font-size:.9em;margin-left:6px}
.rnote{margin:4px 0 0;font-size:.82em;color:var(--muted)}
.rnote.caution{color:var(--warn-fg);background:var(--warn-bg);border-left:4px solid var(--off-line);padding:6px 10px;border-radius:0 4px 4px 0}
...
.legend{background:var(--surface);border:1px solid var(--line);border-radius:8px;margin:14px 0 6px;font-size:.85em;color:var(--head-2)}
.legend>summary{cursor:pointer;padding:10px 14px;font-size:.95em;color:var(--head);
  font-weight:700;user-select:none}
.legend .tbody{padding:0 14px 12px}
.legend dl{margin:0;display:grid;grid-template-columns:auto 1fr;gap:4px 10px}
.legend dt{font-weight:700;color:var(--head);white-space:nowrap}
.legend dd{margin:0}
.legend .rk{display:inline-block;padding:1px 7px;border-radius:10px;font-weight:700;font-size:.85em;margin-right:4px}
.legend .rk-a{background:var(--ok-bg);color:var(--ok-fg)}
.legend .rk-b{background:var(--warn-bg);color:var(--warn-fg)}
.legend .rk-c{background:var(--bad-bg);color:var(--bad-fg)}
/* 足切り表: 見出し帯をオレンジ系にして通常表と視覚的に差別化 */
.tbl.caution table th{background:var(--off-line)}
td.reason{color:var(--warn-fg);font-weight:700;font-size:.85em;white-space:nowrap;text-align:left}
/* 天気アイコン: index.html と同じSVG(#wx-sun 等)を参照。emoji のOS依存表示ズレを回避 */
.wxico{width:1.9em;height:1.9em;display:block;margin:0 auto 2px}
.wxlbl{color:var(--muted);font-size:.82em}
...
#totop{position:fixed;right:16px;bottom:max(16px,env(safe-area-inset-bottom));z-index:30;
  ...
  background:var(--glass);color:var(--head);border:0;border-radius:100px;
  padding:10px;cursor:pointer;font-weight:600;font-size:.82em}
#totop.show{opacity:1;transform:none;pointer-events:auto}
#totop:hover{background:var(--glass-hover)}
#totop .mk{width:20px;height:14px;display:block;margin:0 auto}

/* 「動きを減らす」設定の端末では動きを止める。自動で流れる画面や滑り込む要素は、
   前庭障害のある人には症状の引き金になる。JS のスクロールは scrollBehavior() 側で見る */
@media(prefers-reduced-motion:reduce){
  .find-toggle::after,#totop{transition:none}
}
...
<footer>
<div class="pw-theme" id="themeui"></div>
...
<script>
"use strict";
// 「動きを減らす」設定の端末では瞬時に移動する(index.html と同じ判断)。
// ★ ここを通さない scrollIntoView / scrollTo を増やさないこと(1つ漏れるとそこだけ流れる)。
...
(function(){
  ...
  if(typeof pwGuardPage!=="function"||typeof blockIndex!=="function"){
    document.querySelector("main").innerHTML=
      '<div style="max-width:560px;margin:26px auto;padding:22px 18px;background:var(--surface,#fff);'+
      'border:1px solid var(--line,#dee4ee);border-radius:12px;text-align:center;line-height:1.8">'+
      '<p style="margin:0 0 12px;color:var(--head-2,#44506b)">ページの読み込みに失敗しました。'+
      'お手数ですが、トップページから開き直してください。</p>'+
      '<a href="../index.html" style="display:inline-block;padding:12px 26px;background:var(--btn,#4276b5);'+
      'color:var(--on-night,#fff);border-radius:10px;font-weight:700;text-decoration:none">PeakWeather トップページへ</a></div>';
    return;
  }
  ...
  async function search(fromRestore){
    ...
    try{
      ...
      elResults.scrollIntoView({behavior:scrollBehavior(),block:"start"});
      // 足切り分離した内訳をステータスに出す
