...
(function(){
  ...
  // win=[{hour,code}] (7-15時) → {code,cat,hours,notes}。窓が空なら null。
  // 代表にならなかった降水カテゴリは notes(「昼過ぎに霧雨」等)に降格して情報を落とさない。
  function repWeather(win){
    ...
    // 代表コードが決まったら、それ以外の降水カテゴリを注記に落として返す
    function finish(code){
      var rc=wcat(code),notes=[],c2;
      for(c2 in catHours){
        if(c2===rc||!PRECIP_CATS[c2])continue;
        notes.push(timingLabel(catHours[c2])+"に"+CAT_LABEL[c2]);
      }
      return {code:code,cat:rc,hours:catHours[rc]||[],notes:notes};
    }
  ...
  function dispWx(s){
    ...
    if(rep&&rep.code!=null){
      ...
      if(PRECIP_CATS[cat]&&!drizzleDry)
        return {ic:CAT_ICON[cat], lb:CAT_LABEL[cat], nt:notes};
      // 裏取りで降格した霧雨も注記には残す
      if(drizzleDry&&rep.hours.length)
        notes=notes.concat([timingLabel(rep.hours)+"に"+CAT_LABEL[cat]]);
    }
    // 2) 日照率ベース
    if(f!=null){
      if(f>=0.80)return {ic:"wx-sun",     lb:"よく晴れ", nt:notes};
      if(f>=0.55)return {ic:"wx-suncloud",lb:"晴れ",     nt:notes};
      if(f>=0.30)return {ic:"wx-suncloud",lb:"時々晴れ", nt:notes};
      return       {ic:"wx-cloud",  lb:"曇りがち", nt:notes};
    }
    // 3) 日照率フォールバック: 代表コードのみで大分類
    var c=(rep&&rep.code!=null)?rep.code:s.code;
    if(c==null)return {ic:null,lb:"-",nt:notes};
    if(c===45||c===48)return {ic:"wx-fog",     lb:"霧",          nt:notes};
    if(c===3)         return {ic:"wx-cloud",   lb:"曇り",        nt:notes};
    if(c===2)         return {ic:"wx-suncloud",lb:"晴れ時々曇り", nt:notes};
    return             {ic:"wx-sun",     lb:wlabel(c),     nt:notes};
  }
  ...
  // ---- 検索実行 ----
  // キー接頭辞の数字は保存形式のバージョン。sc の中身を変えたら上げる
  // (find2: 代表天気 wxRep を追加。旧キャッシュを復元すると天気列だけ古い判定に戻るため)。
  function cacheKey(r,p,date){return "find2:"+r+":"+p+":"+date}
  // 直近の検索条件を保存するキー。ページを再訪した時にセレクタと結果を復元する用途
  // (bfcache が効かない iOS 直リンク等のフォールバック。詳細は末尾の restoreLastSearch)。
  var LAST_KEY="find2:last";
  ...
  var LEGEND_HTML=(
    ...
    '<dt>天気</dt><dd>7〜15時 の代表天気。<b>時間数の多数決</b>で決め、'+
      '強い雨・雪・雷は1時間でもあれば優先表示する。降水が代表でない日は日照率で'+
      '「よく晴れ・晴れ・時々晴れ・曇りがち」を判定し、代表になれなかった短時間の降水は'+
      '「昼過ぎに霧雨」のように下段へ小さく注記する</dd>'+
