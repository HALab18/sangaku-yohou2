...
そこで **キーごと・日ごとに非null件数を数え**、既知の前提を固定する:
  ...
  6. JMA_DAYS を超えて要求してもエラーにならず、11日目は**部分日**になって尽きる
     (JMA_DAYS=10 は「APIが10日しか返さない」からではなく、11日目の日別集計が作れないから)
...
def main():
    ...
    if d4 is None:
        ...
        h4 = d4.get("hourly") or {}
        t4 = h4.get("time") or []
        # ★ JMA_DAYS=10 は「APIが10日しか返さないから」ではない。モデル自体は11日目まで
        #   配信するが、**11日目は昼過ぎで切れて日別集計が丸ごと null になる**ので、
        #   完全にデータが揃う10日目で表示を止めている。ここで確かめるのはその前提:
        #   「超えて要求してもエラーにならない」ことと「どこかで必ず尽きる」こと。
