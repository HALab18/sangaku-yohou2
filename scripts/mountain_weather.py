...
# **AT は「乾いた状態」の式で、濡れた衣服の蒸発冷却を含まない。** 体感の数字そのものは
# この帯でも冷えを表すが(飯豊山の気温13.8℃・稜線風17m/s は AT で約2〜3℃)、
# 濡れるとそこからさらに下がる。数字が一桁台に見えても致命的にならない帯こそ
# 濡れ+風の低体温が起きるので、ここは印で補う。
# ※旧実装(風冷指数 JAG/TI)は10℃超で適用外になり「体感=気温」を返していたため、
#   同じケースが体感13.8℃と表示されていた。AT への置換でこの欠陥自体は解消済み。
# 判定(D1)のしきい値は 10℃ のまま変えていない
...
def main():
    ...
    def emit():
        ...
        print("- 体感温度 = 豪州気象局の Apparent Temperature (Steadman式。気温・風・湿度から算出。"
              "日射は考慮しないため日なたでは実際より涼しく出ます。材料が1つでも欠測なら「-」)。"
