...
**現状**: 下記「標高100m未満は稜線風ではなく地上10m風を使う」が**未 push**。
ローカル(http://localhost:8765)で動作確認済み・`check_mountains.py` OK・
富士山のCLI出力は変更前とバイト一致（100m以上のリグレッションなし）。

**次にやること**: 実機で「📍 現在地の予報を見る」を街中で押して注記と風速を確認し、
問題なければ `git push origin <ブランチ>:master`。
※ その前段の「山さがしの再訪挙動」も未 push のまま残っている（下記）。

---

## 標高100m未満は稜線風ではなく地上10m風を使う（2026-07-30）
...
## （前セッションの再開ポイント）山さがしの再訪挙動を公開する（2026-07-30）
