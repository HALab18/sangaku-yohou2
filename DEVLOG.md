...
## ▶ 次の再開ポイント: history.html の項目漏れを整理する（2026-07-30）

**現状**: `ver 2.13β`（低標高の地上風対応）は **公開済み**（push 完了）。
ローカル master = origin/master（bfbb406）。

**宿題**: `docs/history.html` に版番号表記の漏れがあり：
- **7/29 版**: 2e500a0「山さがしは一覧から戻った時だけ検索結果を復元する」は公開済みなのに
  改訂履歴に項目が無い
- 2.13β については既に history.html に追加済み

**判断待ち**: 7/29 版の項目を history.html に追加するかどうか。次に history.html を
修正する際に判定する。
