...
## ▶ 次の再開ポイント: スコア刷新 + find/index の UI 改善 (山名隣スコア・県必須・副次ボタン・丁寧語化)
...
- **2aa12af→d73441f index.html: find.html への入口をフォーム内ボタンに一本化**
  一旦ヒーローに副次ボタンを置いたが (2aa12af)、ユーザー指示で d73441f にて
  フォーム内へ移設。「予報を見る」と「現在地の予報を見る」の間に、#gps と同じ
  セカンダリ様式のリンクボタン `#findbtn` を配置。ヒーローの `.hero-find`、
  入力欄上の「天気で山さがし ▸」(.findrow)、フッターの同リンクはすべて削除し、
  **find.html への入口はフォーム内ボタン1つに一本化**。find.html から個別予報に
  遷移した際の「一覧に戻る」導線 (pw_from_find) は従来どおり維持。
