...
- `run(mt)` の `Promise.all` 直後に `const fetchedAt=new Date();` を置き、詳細ループの直前で
  `capMeta` を1回だけ組んで各表の `<h2>` 直下に差し込む。日付整形は既存の `iso()` を再利用。
- **週間表は触っていない**（直上に山名見出しと `ul.meta` があり情報が落ちないため）。
