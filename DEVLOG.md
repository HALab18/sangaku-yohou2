...
## ▶ 次の再開ポイント: 低標高の地上風対応を公開する（2026-07-30）

**現状**: 下記「標高100m未満は稜線風ではなく地上10m風を使う」を **ver 2.13β** として
コミットし、**ローカル master には fast-forward で反映済み。ただし未 push**
（`origin/master` は 2e500a0 のまま）。動作確認済み: 利用者のPC Chrome で
現在地（仙台市・標高69m）が「地上風(10m)」表示になり指数も改善・`check_mountains.py` OK・
富士山のCLI出力は変更前とバイト一致（100m以上のリグレッションなし）。

**次にやること**: 実機（スマホ）で「📍 現在地の予報を見る」を街中で押して注記と風速を確認し、
問題なければ `git push origin master`。

**未反映の宿題**: `docs/history.html` に**7/29 の版番号表記が無い**（2.12β 相当の変更のはず）。
また **2e500a0「山さがしは一覧から戻った時だけ検索結果を復元する」は公開済みなのに
改訂履歴に項目が無い**。次に history.html を触るときに整理するかを判断する。

---

## 標高100m未満は稜線風ではなく地上10m風を使う / ver 2.13β（2026-07-30）

**未公開**（ローカル master には反映済み・push 前）。
