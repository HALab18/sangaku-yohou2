...
- **2aa12af index.html: ヒーローに「天気の良い山をさがす」副次ボタンを追加**
  「予報をチェックする」の直下に find.html への入口ボタン (`.hero-find`)。
  主目的は個別山の予報なので、副次ボタンは背景/枠/文字とも薄い配色。
  下向き矢印 (.chev) は hero-cta から独立させて最下部へ。
